...
try:
    from .resume_document import DESIGN_PATH, build_resume_sections, dump_yaml, load_yaml
except ImportError:  # Support direct execution as scripts/build.py.
    from resume_document import DESIGN_PATH, build_resume_sections, dump_yaml, load_yaml
...
def build_document(profile: dict[str, Any], variant: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "cv": {
            ...
            "sections": build_resume_sections(profile, variant),
        },
        "design": design,
...
def render(path: Path) -> None:
    ...
    if not rendercv.exists():
        raise RuntimeError("RenderCV is not installed. Run `make setup` first.")
